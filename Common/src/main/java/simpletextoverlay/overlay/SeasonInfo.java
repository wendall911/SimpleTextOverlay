...
public class SeasonInfo extends Info {
    ...
    public void renderText(PoseStack matrix, Minecraft mc, BlockPos pos, int scaledWidth, int scaledHeight) {
        Pair<TranslatableComponent, SubSeason> seasonInfo = Services.SEASON_INFO.getSeasonName(mc.level, pos);
        ...
        if (seasonInfo != null) {
            ...
            FontHelper.draw(mc, matrix, seasonName, x, y, ColorHelper.getSeasonColor(subSeason), FontHelper.TextType.VALUE);
        }
    }
}
